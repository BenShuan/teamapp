...
export const Route = createFileRoute("/(app)/serialized-gear/check")({
...
function SerializedGearCheckPage() {
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return assignments.filter((a: SerializedGearFighter) => a.fightersTeamId === teamId);
  ...
  useEffect(() => {
    ...
    for (const a of filteredAssignments) {
      ...
      const hasCheck = Array.isArray(a.checks) && a.checks.some((c: any) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Autocomplete
              ...
              options={Object.values(teamsMap).map((t) => ({ label: t.label, value: t.value.id }))}
              value={teamId}
                ...
                {items.map((a) => (
                  <div key={a.id} className="flex items-center justify-between p-3 rounded border">
                    <div className="text-right">
                      <div className="font-medium">{`${catalogMap[a.serializedGearId].label} - ${a.serialNumber} ` || "—"}</div>
                      <div className="text-sm text-muted-foreground">לוחם: {fightersMap[a.fighterId].label || "—"}</div>
                      <div className="text-sm text-muted-foreground">מיקום: {a.location || "—"}</div>
                    </div>
                    <Checkbox checked={!!checkedIds[a.id]} onCheckedChange={() => toggleChecked(a.id)} />
                  </div>
                ))}
              </div>
            </div>
